async function addLocker(event){
    ...
    tData = document.querySelector('#lockerTable')
    ...
    if('error' in result){
        ...
        <td><a href="/locker/${result.locker_code}"> ${result.locker_code} </a> </td>
        <td> <a href="/key/${result.key}">${result.key} <a></td>
        <td><a href="/area/${result.area}">${result.area}</a></td>
        <td>${result.locker_type}</td>
        <td>${result.status}</td>
        <div style=" position: relative;display: inline-flex;">
                <td>
                    <a class="dropdown-trigger btn-floating purple darken-4 waves-effect" data-target="${result.locker_code}_dropdown1"><i class="large material-icons">list</i></a>
                    <ul id='${result.locker_code}_dropdown1' class='dropdown-content purple darken-4 white-text'>`
            if(result.status === "Free"){
                tData += `<li><a href="/locker/rent/${result.locker_code}/student" class="white-text"><i class="material-icons left white-text">check</i>Rent</a></li>`
            }
            else if (result.status === "Rented"){
                tData += ` <li><a href="#" onclick="loadComments({},1)" class="white-text"><i class="material-icons left white-text">check</i>Note</a></li>
                <li><a href="#" onclick="releaseMode({})" class="white-text"><i class="material-icons left white-text">close</i>Release</a></li>`
            }

            else if (result.status == "Not Verified"){
                tData += `<li><a href="#" onclick="loadComments({}},1)" class="white-text"><i class="material-icons left white-text">check</i>Note</a></li>
                <li><a href="/rent/1/release/verify" class="white-text"><i class="material-icons left white-text">verified_user</i>Verify</a></li>`
            }
            tData +=`
                <li><a href="#edit" onclick="editMode({})" class="white-text"><i class="material-icons left white-text">edit</i>Edit</a></li>
                <li><a href="#delete" onclick="removeMode({})" class="white-text"><i class="material-icons left white-text">delete</i>Delete</a></li>
...
async function updateLocker(event){
    ...
    if('error' in result){
        ...
        <td>${result.status}</td>
        <td> <a href="/key/${result.key}">${result.key} <a></td>
        <td><a href="/area/${result.area}">${result.area_description}</a></td>
        <td>
        <a class="dropdown-trigger btn-floating purple darken-4 waves-effect" data-target="${result.locker_code}_dropdown1"><i class="large material-icons">list</i></a>
        <ul id='${result.locker_code}_dropdown1' class='dropdown-content purple darken-4 white-text'>`;
        if(result.status === "Free"){
            html += `<li><a href="/locker/rent/${result.locker_code}/student" class="white-text"><i class="material-icons left white-text">check</i>Rent</a></li>`;
        }
        else if (result.status === "Rented"){
            html += ` <li><a href="#" onclick="loadComments(${result.current_rental.id},1)" class="white-text"><i class="material-icons left white-text">check</i>Note</a></li>
            <li><a href="#" onclick="releaseMode('${result.current_rental}')" class="white-text"><i class="material-icons left white-text">close</i>Release</a></li>`;
        }

        else if (result.status == "Not Verified"){
            html += `<li><a href="#" onclick="loadComments(${result.current_rental.id},1)" class="white-text"><i class="material-icons left white-text">check</i>Note</a></li>
            <li><a href="/rent/${result.current_rental.id}/release/verify" class="white-text"><i class="material-icons left white-text">verified_user</i>Verify</a></li>`;
        }
        html +=`
            <li><a href="#edit" onclick="editMode({'locker_code':'${result.locker_code}','locker_type':'${result.locker_type}','area':${result.area},'key':'${result.key}','status':'${result.status}'})" class="white-text"><i class="material-icons left white-text">edit</i>Edit</a></li>
            <li><a href="#delete" onclick="removeMode({'locker_code':'${result.locker_code}','locker_type':'${result.locker_type}','area':${result.area},'key':'${result.key}','status':'${result.status}'})" class="white-text"><i class="material-icons left white-text">delete</i>Delete</a></li>
            <li><a href="#delete" onclick="OpenswapKey('')" class="white-text"><i class="material-icons left white-text">swap_horiz</i>Swap</a></li>
        </ul>
        </td>`;
